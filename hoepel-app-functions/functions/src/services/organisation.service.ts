...
export class OrganisationService {
   ...
   */
  async listMembers(organisationId: string): Promise<ReadonlyArray<any>> {
    ...
    const members: ReadonlyArray<{ belongsToTenant: boolean, user: any, permissions: ReadonlyArray<string> }> = await Promise.all(
      allUsers.docs.map(userDoc => userDoc.ref.collection('tenants').doc(organisationId).get().then(tenantDoc => {
        return {
   ...
   */
  async listPossibleMembers(organisationId: string): Promise<ReadonlyArray<any>> {
    // TODO should filter out users that are already part of organisation
    return (await this.db.collection('users').get()).docs.map(user => ({ ...user.data(), uid: user.id }));
  }
   ...
   */
  async requestCreateNewOrganisation(organisation: Tenant, user: Object): Promise<void> {
