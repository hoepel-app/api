...
export const onUserDelete = functions.region('europe-west1').auth.user().onDelete(async user => {
  ...
  await nodemailerMailgun.sendMail({
    ...
    text: `Een persoon heeft zijn/haar account verwijderd. Naam: ${user.displayName || '<geen naam>'}, contact: ${user.email}. Details als bijlage. Verwijder de gebruiker en sporen daarvan.`,
    attachments: [{
