...
export const resolvers: IResolvers = {
  Mutation: {
    ...
    changeDisplayName: async (obj, { name }: { name: string }, context: Context, info) => {
      new AuthorizationService(context).assertLoggedIn()
      await userService.updateDisplayName(context.token.uid, name)
    },
    deleteReport: async (obj, { tenant, fileName }: { tenant: string, fileName: string }, context: Context, info) => {
      new AuthorizationService(context).assertLoggedIn()
      await assertHasPermission(context.token.uid, tenant, Permission.ReportsDelete)
      await fileService.removeFile(tenant, context.token.uid, fileName)
    },
    createReport: async (obj, { tenant, type, format, year }: { tenant: string, type: string, format: string, year?: number }, context: Context, info) => {
      new AuthorizationService(context).assertLoggedIn()
      await assertHasPermission(context.token.uid, tenant, Permission.ReportsRequest)
      ...
      switch(type) {
        case 'all-children':
          return await fileService.exportAllChildren(tenant, createdBy, context.token.uid);
        case 'all-crew':
          return await fileService.exportAllCrew(tenant, createdBy, context.token.uid);
        case 'children-with-comment':
          return await fileService.exportChildrenWithComment(tenant, createdBy, context.token.uid);
        case 'crew-attendances':
          return await fileService.exportCrewAttendances(tenant, createdBy, context.token.uid, reportYear);
        case 'child-attendances':
          return await fileService.exportChildAttendances(tenant, createdBy, context.token.uid, reportYear);
        case 'fiscal-certificates-list':
          return await fileService.exportFiscalCertificatesList(tenant, createdBy, context.token.uid, reportYear);
        case 'children-per-day':
          return await fileService.exportChildrenPerDay(tenant, createdBy, context.token.uid, reportYear);
        default:
          throw new Error(`No exporter found for ${type}`)
      }
    },
    testTemplate: async (obj, args: { tenant: string, templateFileName: string }, context: Context, info) => {
      new AuthorizationService(context).assertLoggedIn()
      await assertHasPermission(context.token.uid, args.tenant, Permission.TemplateRead)
    ...
    fillInTemplate: async (obj, args: { tenant: string, childId: string, templateFileName: string, year?: number }, context: Context, info) => {
      new AuthorizationService(context).assertLoggedIn()
      await assertHasPermission(context.token.uid, args.tenant, Permission.TemplateFillIn)
      await assertHasPermission(context.token.uid, args.tenant, Permission.ChildRead)
    ...
    deleteTemplate: async (obj, args: { tenant: string, templateFileName: string }, context: Context, info) => {
      new AuthorizationService(context).assertLoggedIn()
      await assertHasPermission(context.token.uid, args.tenant, Permission.TemplateWrite)
