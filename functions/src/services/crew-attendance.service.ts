...
export type ICrewAttendanceByCrewRepository = TenantIndexedRepository<CrewAttendancesByCrewDoc & { id: string }>;
export type ICrewAttendanceByShiftRepository = TenantIndexedRepository<CrewAttendancesByShiftDoc & { id: string }>;

export const createCrewAttendanceByCrewRepository = (db: admin.firestore.Firestore): ICrewAttendanceByCrewRepository => {
...
export const createCrewAttendanceByShiftRepository = (db: admin.firestore.Firestore): ICrewAttendanceByShiftRepository => {
...
export class CrewAttendanceService {
  ...
  async getAttendancesForCrew(tenant: string, crewId: string): Promise<{ [p: string]: IDetailedCrewAttendance }> {
    try {
  ...
  async getCrewAttendancesOnShifts(tenant: string, shifts: ReadonlyArray<IShift>): Promise<ReadonlyArray<{
    shiftId: string,
