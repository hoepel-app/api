...
import { Address, Child, ContactPerson, Crew, IChild, Price, Shift } from '@hoepel.app/types';

describe('XlsxExporter', async () => {
  const exporter = new XlsxExporter();

  const child1 = Child.empty().withFirstName('Kind 1').withLastName('Achter').withId('child-id-1')
    .withBirthDate({day: 2, month: 3, year: 2014})
    .withPhoneContact([{phoneNumber: '+1111'}, {phoneNumber: '+2222', comment: 'comment'}])
    .withEmail(['aoeu@test.com', 'test@hello.com'])
    .withGender('male')
    .withUitpasNumber('11-1111-11')
    .withRemarks('Remarks here')
    .withAddress(new Address({street: 'Straat', number: '12A', city: 'Stad', zipCode: 4444}));
  const child2 = Child.empty().withFirstName('Kind 2').withLastName('Achter').withId('child-id-2')
    .withContactPeople([ { relationship: 'Mama', contactPersonId: 'contact-id-1' } ]);
  const child3 = Child.empty().withFirstName('Kind 3').withLastName('Achter').withId('child-id-3');

  const crew1 = Crew.empty().withId('crew-id-1').withFirstName('Animator 1').withLastName('Achernaam')
    .withBirthDate({ day: 2, month: 3, year: 2014 })
    .withPhoneContact([ { phoneNumber: '+2132' }, { phoneNumber: '+4321', comment: 'comment' } ])
    ...
    .withRemarks('Remarks here');
  const crew2 = Crew.empty().withId('crew-id-2').withFirstName('Animator 2').withLastName('Achernaam')
    .withCertificates({ hasPlayworkerCertificate: true, hasTrainerCertificate: false, hasTeamleaderCertificate: false });
  const crew3 = Crew.empty().withId('crew-id-3').withFirstName('Animator 3').withLastName('Achernaam');
  ...
  const shift1 = new Shift({
    ...
    startAndEnd: { start: { hour: 9, minute: 0 }, end: { hour: 12, minute: 0 } },
  });
  const shift2 = new Shift({
    ...
    startAndEnd: { start: { hour: 13, minute: 0 }, end: { hour: 17, minute: 30 } },
  });
  ...
  const contactPerson1 = ContactPerson.empty().withId('contact-id-1')
    .withFirstName('Mieke').withLastName('Contact')
    .withAddress({ street: 'Contactstraat', number: '123', zipCode: 7777, city: 'Stadt' });
  const contactPerson2 = ContactPerson.empty().withId('contact-id-2')
    .withFirstName('Mieke').withLastName('Contact');
  ...
  it('createChildList', () => {
    ...
    const childListExpected = [
      ...
      {'values': ['Geboortedatum', {day: 2, month: 3, year: 2014}, undefined, undefined], 'width': 15},
  ...
  it('createChildrenWithCommentList', () => {
    ...
    const childListExpected = [
      ...
      {'values': ['Geboortedatum', {day: 2, month: 3, year: 2014} ], 'width': 15},
      {'values': ['Telefoonnummer', '+1111, +2222 (comment)'], 'width': 25},
      {'values': ['Emailadres', 'aoeu@test.com, test@hello.com'], 'width': 25},
      {'values': ['Adres', 'Straat 12A, 4444 Stad'], 'width': 30},
      {'values': ['Gender', 'Man']},
  ...
  it('createCrewMemberList', () => {
    ...
    const crewListExpected = [
      ...
      {'values': ['Familienaam', 'Achernaam', 'Achernaam', 'Achernaam'], 'width': 25},
      {'values': ['Geboortedatum', {day: 2, month: 3, year: 2014}, undefined, undefined], 'width': 15},
      {'values': ['Telefoonnummer', '+2132, +4321 (comment)', '', ''], 'width': 25},
      {'values': ['Emailadres',  'aoeu@test.com, hello@example.com' , '', ''], 'width': 25},
      {'values': ['Adres', 'Straat 12A, 4444 Stad', '', ''], 'width': 30},
      {'values': ['Actief', 'Nee', 'Ja', 'Ja']},
      {'values': ['Rekeningnummer', 'BE44 4444 4444', '', ''], 'width': 25},
      {'values': ['Gestart in', 2019, undefined, undefined]},
      {'values': ['Attesten', 'Attest animator, Attest hoofdanimator, Attest instructeur', 'Attest animator', ''], 'width': 35},
      {'values': ['Opmerkingen', 'Remarks here', '', ''], 'width': 75},
    ];
  ...
  it('createCrewMembersAttendanceList', () => {
    const res = exporter.createCrewMembersAttendanceList(
      [crew1, crew2, crew3],
      [shift2, shift1, shift4],
      [
        { shiftId: 'shift-id-2', attendances: { 'crew-id-2': { didAttend: true },  'crew-id-1': { didAttend: true }, 'crew-id-3': { didAttend: false }  } },
        { shiftId: 'shift-id-1', attendances: { 'crew-id-1': { didAttend: true } } },
      ],
      2019,
      );
    ...
    const expected = [
      ...
      {'values': [{'day': 3, 'month': 4, 'year': 2019}, 'Voormiddag', 'Omschrijving 1', true, false], 'width': 22},
      {'values': [{'day': 3, 'month': 4, 'year': 2019}, 'Namiddag', 'Omschrijving 2', true, true], 'width': 22},
      { 'values': [{ 'day': 5, 'month': 8, 'year': 2019, }, 'Externe activiteit', 'Bellewaerde met de tieners', false, false], 'width': 22},
    ];
      ...
      [
        {
          shiftId: 'shift-id-2',
          attendances: {
            'child-id-2': { didAttend: true, amountPaid: { euro: 5, cents: 0 } },
            'child-id-1': { didAttend: true, amountPaid: { euro: 6, cents: 0 } },
            'child-id-3': { didAttend: false, amountPaid: { euro: 2, cents: 50 } },
          }
        },
        {
          shiftId: 'shift-id-1',
          attendances: { 'child-id-1': { didAttend: true, amountPaid: { euro: 7, cents: 20 } } }
        },
      ],
      2019,
      );
  ...
  it('createAllFiscalCertificates', () => {
    ...
    const expected = [
      {'values': ['', '', '', 'Voornaam', 'Kind 1', 'Kind 2'], 'width': 20},
      {'values': ['', '', '', 'Familienaam', 'Achter', 'Achter'], 'width': 25},
      {'values': ['', '', '', 'Totaal (incl. korting)', { euro: 13, cents: 20 }, { euro: 5, cents: 0 } ], 'width': 25},
      {'values': ['', '', '', 'Geboortedatum', { day: 2, month: 3, year: 2014 }, undefined ], 'width': 25},
      {'values': ['', '', '', 'Contactpersoon', '', 'Mieke Contact' ], 'width': 25},
      {'values': ['', '', '', 'Straat en nummer', 'Straat 12A', 'Contactstraat 123' ], 'width': 25},
      {'values': ['', '', '', 'Postcode', 4444, 7777 ], 'width': 25},
      {'values': ['', '', '', 'Stad', 'Stad', 'Stadt' ], 'width': 25},
      {'values': ['Dag', 'Type', 'Prijs' ], 'width': 25},
      {'values': [{'day': 3, 'month': 4, 'year': 2019}, 'Voormiddag', { euro: 5, cents: 50 }, 'Omschrijving 1', true, false], 'width': 22},
      {'values': [{'day': 3, 'month': 4, 'year': 2019}, 'Namiddag', { euro: 2, cents: 50 }, 'Omschrijving 2', true, true], 'width': 22},
      { 'values': [{ 'day': 5, 'month': 8, 'year': 2019, }, 'Externe activiteit', { euro: 20, cents: 0 }, 'Bellewaerde met de tieners', false, false], 'width': 22},
    ];
