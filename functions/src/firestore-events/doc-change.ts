...
import { Collection, FirestoreCollectionEvents, IEvent, store } from '@hoepel.app/types';
 ...
 */
const getTenant = (collectionId: string, beforeDoc?: any, afterDoc?: any): string | undefined => {
 ...
 */
const firestoreEventCreators: ReadonlyArray<FirestoreCollectionEvents<any>> = Object.entries(store)
  .map(( [ field, collection]: [ string, Collection<any> ] ) => {
 ...
 */
const getContextIds = (collectionId: string, documentId: string, entity: any): {
  shiftId?: string,
  childId?: string,
  crewId?: string,
 ...
 */
const getContextIdsForEvent = (type: 'updated' | 'created' | 'deleted', collectionId: string, documentId: string, before: any, after: any) => {
  switch (type) {
    case 'created':
...
const createEvent = (
  ...
  if (!tenant) {
    throw new Error(`Could not get tenant for event! Document: ${collectionId}/${documentId}`);
  }

  const eventCreator = firestoreEventCreators.find(creator => creator.isAppropriateFor(collectionId));
...
const handleDocumentChange = async (
  ...
  if (event) {
    ...
    throw new Error(`Could not create an event for ${collectionId}/${documentId} (operation: ${type})!`);
  }
};

export const onDocumentUpdate = functions.region('europe-west1').firestore.document('{collectionId}/{documentId}')
...
export const onDocumentDelete = functions.region('europe-west1').firestore.document('{collectionId}/{documentId}')
...
export const onDocumentCreate = functions.region('europe-west1').firestore.document('{collectionId}/{documentId}')
